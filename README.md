...
- Copy **`.env.example`** to **`.env.local`**. **`REACT_APP_API_URL`** is the backend origin only (default in code: **`https://api.ruvali.co.in`**), **not** `…/api` — callers use **`/api/...`** paths. The storefront can stay on **`https://ruvali.co.in`** while the API lives on **`REACT_APP_API_URL`**.
